...
class BaseTrainer:
            ...
            if batch_idx % self.log_step == 0:
                ...
                self.logger.debug(
                    "Train Epoch: {} {} Loss: {:.6f}".format(
                        epoch, self._progress(batch_idx), batch["loss_gener"].item()
                    )
                )
                self.writer.add_scalar(
                    "learning rate", self.lr_scheduler["gen"].get_last_lr()[0]
                )
    ...
    def _save_checkpoint(self, epoch, save_best=False, only_best=False):
        ...
        state = {
            ...
            "optimizer": [
                self.optimizer["gen"].state_dict(),
                self.optimizer["disc"].state_dict(),
            ],
            "lr_scheduler": [
                self.lr_scheduler["gen"].state_dict(),
                self.lr_scheduler["disc"].state_dict(),
            ],
